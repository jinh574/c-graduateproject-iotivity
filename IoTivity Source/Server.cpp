...
class s_callback : public virtual mqtt::callback, public virtual mqtt::iaction_listener
{
	...
	virtual void message_arrived(const std::string& topic, mqtt::const_message_ptr msg) {
		...
		{
			const Json::Value data = root["data"];
			delete_sensor = root["data"]["sensor_id"].asString();
			isDelete = 1;
		}
	}
...
int main(int argc, char* argv[])
{
	...
	try
	{
		...
		while(1)
		{
			...
			sensorPAYLOAD = "{\"controller_id\":\"" + controllerID + "\", \"controller_name\":\"" + controllerNAME + "\", \"update_timestamp\":" + std::to_string(time_now)  + ", \"admin_name\":\"" + adminNAME + "\", \"admin_tel\":\"" + adminTEL + "\", \"description\":\"" + description + "\", \"sensor_data\":[";
			int tempCnt = 1;
			...
			if(isDelete)
			{
				for(unsigned int i = 0; i < v.size(); i++)
				{
					if((v[i]->m_name).compare(delete_sensor)==0)
				    {
						v.erase(v.begin()+i-1);
						isDelete = 0;
					}
